...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        d_loss_tf = self.model.d_loss_metric.result()
        g_loss_tf = self.model.g_loss_metric.result()
        x.append(float(str(d_loss_tf)[10:16]))
        y.append(float(str(g_loss_tf)[10:16]))
        ...
        for i in range(self.num_img):
            ...
            plt.imshow(img)
            plt.axis('off')
        fig1.suptitle("Discriminator loss: "+str(d_loss_tf)[10:16] +"\nGenerator loss: " + str(g_loss_tf)[10:16])
        fig1.savefig('epoch_{:03d}.png'.format(epoch))
        plt.close(fig)
        #plt.show()
    ...
    def on_train_end(self, logs=None):
        ...
        # print(x)
        fig2 = plt.figure(2)
        w = list(range(NUM_EPOCHS))
        plt.plot(w, x, label="Discriminator")
        plt.plot(w, y, label="Generator")
        plt.xlabel("Number of epoch")
        plt.suptitle("")
        plt.legend()
        fig2.savefig('DGLosses.png')
        plt.close(fig2)
       # plt.show()
